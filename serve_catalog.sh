...
ROOT_PHOTOS="${ROOT_PHOTOS:-/Users/alvarotrabanco/Photography/les_mios_semeyes}"

OUT_DIR="${OUT_DIR:-out}"
SITE_DIR="${SITE_DIR:-$OUT_DIR/site}"
CSV_PATH="${CSV_PATH:-$OUT_DIR/archive.csv}"
JSON_PATH="${JSON_PATH:-$SITE_DIR/catalog.json}"
THUMBS_DIR="${THUMBS_DIR:-$SITE_DIR/thumbs}"
JOBS="${JOBS:-0}"

# Force-refresh HTML each run
HTML_MODE="force"

SCRIPT_DIR="$(cd -- "$(dirname "$0")" && pwd -P)"
cd "$SCRIPT_DIR"

mkdir -p "$SITE_DIR" "$THUMBS_DIR"

echo "== Rebuilding catalog & HTML (reusing thumbnails) =="
...
ls -ld "$THUMBS_DIR" || echo "(no thumbs dir yet)"
